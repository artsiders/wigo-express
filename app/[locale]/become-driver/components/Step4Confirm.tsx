...
function SummaryRow({ label, value }: { label: string; value?: string | number }) {
  ...
  return (
    <div className="flex justify-between items-center py-3 border-b border-neutral-100 last:border-0 group">
      <span className="text-[10px] font-black text-neutral-400 uppercase tracking-widest group-hover:text-neutral-500 transition-colors">{label}</span>
  ...
  return (
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="space-y-2 text-center">
        ...
        <h2 className="text-4xl font-black text-dark tracking-tighter">Récapitulatif</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div className="space-y-6">
          {/* Personal */}
          <div className="bg-neutral-50/50 rounded-xl p-6 border border-neutral-100 shadow-sm hover:shadow-md transition-shadow">
            <div className="flex items-center gap-3 mb-4">
              <div className="w-8 h-8 rounded-lg bg-primary/10 flex items-center justify-center">
                 <LuUser size={16} className="text-primary" />
              </div>
              <span className="text-xs font-black text-dark uppercase tracking-widest">Profil</span>
            </div>
          ...
          <div className="bg-neutral-50/50 rounded-xl p-6 border border-neutral-100 shadow-sm hover:shadow-md transition-shadow">
            <div className="flex items-center gap-3 mb-4">
              ...
              <span className="text-xs font-black text-dark uppercase tracking-widest">Permis</span>
            </div>
            ...
            <div className="flex justify-between items-center pt-3 mt-1">
              <span className="text-[10px] font-black text-neutral-400 uppercase tracking-widest">Document</span>
              {data.licenseDocumentUrl ? (
                <div className="flex items-center gap-2 bg-green-500/10 px-3 py-1 rounded-full">
                  <span className="text-[10px] font-black text-green-600 uppercase tracking-tight">Vérifié</span>
                  <LuCheck size={12} className="text-green-600" />
                </div>
              ) : (
                <span className="text-[10px] text-red-500 font-black uppercase">Manquant</span>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6">
          {/* Vehicle */}
          <div className="bg-neutral-50/50 rounded-xl p-6 border border-neutral-100 shadow-sm hover:shadow-md transition-shadow h-full">
            <div className="flex items-center gap-3 mb-4">
              <div className="w-8 h-8 rounded-lg bg-primary/10 flex items-center justify-center">
                 <LuCar size={16} className="text-primary" />
              </div>
              <span className="text-xs font-black text-dark uppercase tracking-widest">Véhicule</span>
            </div>
            ...
            <SummaryRow label="Capacité" value={data.vehicleSeats ? `${data.vehicleSeats} places` : undefined} />
          </div>
        </div>
      </div>
      ...
      <div className="pt-4">
        ...
        >
          ...
          <div>
            <p className="text-base font-black text-dark tracking-tight leading-none mb-2">
              Engagement et assurance
            </p>
            <p className="text-sm text-neutral-500 font-medium leading-relaxed">
              Je certifie l'exactitude des informations fournies et confirme que mon véhicule est en règle et correctement assuré pour le covoiturage.
            </p>
          </div>
        </label>
        ...
        {errors.acceptTerms && (
          <div className="mt-4 p-3 bg-red-50 rounded-xl border border-red-100 flex items-center gap-2 text-red-500 animate-in slide-in-from-top-1">
            ...
            <p className="text-xs font-bold uppercase tracking-widest">{errors.acceptTerms.message}</p>
          </div>
        )}
      </div>
    </div>
