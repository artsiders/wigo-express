...
export function StepIdentityVerso() {
  ...
  const currentUrl = watch("versoUrl");
  ...
  const onDrop = useCallback(
    async (acceptedFiles: File[]) => {
      ...
      try {
        ...
        setValue("versoUrl", result.url, { shouldValidate: true });
      } catch {
        setPreview(null);
      }
    },
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-300">
      <div className="space-y-2 text-center">
        <div className="w-16 h-16 bg-primary/10 rounded-2xl flex items-center justify-center mx-auto mb-4">
          <LuCreditCard size={32} className="text-primary rotate-180" />
        </div>
        <h2 className="text-3xl font-black text-dark tracking-tight">Verso de la pièce d'identité</h2>
        <p className="text-neutral-500 font-medium max-w-sm mx-auto">
          Prenez en photo le dos de votre pièce d'identité.
        </p>
      </div>
      ...
      <div className="space-y-4">
        {currentUrl && preview ? (
          <div className="relative rounded-3xl overflow-hidden border-4 border-green-400/30 bg-neutral-50 shadow-2xl">
            <img src={preview} alt="Verso" className="w-full aspect-[1.6/1] object-cover" />
            <div className="absolute inset-0 bg-linear-to-t from-black/60 via-transparent to-transparent flex items-end p-6">
              <div className="flex items-center gap-3 text-white">
                ...
                <span className="text-sm font-black uppercase tracking-widest">Document prêt</span>
              </div>
            </div>
            <button
              type="button"
          ...
          <div
            {...getRootProps()}
            className={`group relative aspect-[1.6/1] border-4 border-dashed rounded-3xl flex flex-col items-center justify-center text-center cursor-pointer transition-all duration-500 overflow-hidden ${
              isDragActive
            ...
            {uploadMutation.isPending ? (
              ...
              <>
                ...
                <p className="text-lg font-black text-neutral-800 tracking-tight">
                  {isDragActive ? "Lâchez ici !" : "Uploader le Verso"}
                </p>
        ...
        <div className="text-center">
          <FieldError message={errors.versoUrl?.message} />
        </div>
      </div>
    </div>
  );
