          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                ...
                <input
                  ...
                  className="w-full p-4 bg-zinc-50 border border-zinc-200 rounded-xl focus:ring-2 focus:ring-primary focus:outline-none"
                  value={form.make}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  className="w-full p-4 bg-zinc-50 border border-zinc-200 rounded-xl focus:ring-2 focus:ring-primary focus:outline-none uppercase"
            ...
            <div className="space-y-2">
              ...
              <select
                ...
                onChange={(e) =>
                  setForm({ ...form, seatsCapacity: parseInt(e.target.value) })
                }
              >
            ...
            {addVehicle.error && (
              <div className="p-4 bg-red-50 text-red-600 border border-red-100 rounded-xl text-sm font-medium">
                {addVehicle.error instanceof Error
                  ? "Erreur : ce véhicule existe peut-être déjà."
                  : "Erreur inattendue."}
              </div>
            )}
          ...
          {!profile.vehicles || profile.vehicles.length === 0 ? (
            <div className="text-center py-20 bg-white rounded-2xl border border-zinc-100 shadow-[0_20px_40px_rgba(0,0,0,0.02)]">
              ...
              <p className="text-zinc-500 max-w-sm mx-auto">
                Vous n'avez pas encore renseigné de véhicule. Ajoutez-en un pour
                pouvoir publier des trajets !
              </p>
            </div>
          ) : (
            profile.vehicles.map((v) => (
              <div
                key={v.id}
                className="bg-white rounded-2xl p-6 md:p-8 flex flex-col md:flex-row md:items-center justify-between gap-6 border border-zinc-200 shadow-[0_20px_40px_rgba(0,0,0,0.02)] relative overflow-hidden group"
              >
                <div className="flex items-center gap-6">
                  ...
                  <div>
                    ...
                    <p className="text-zinc-500 font-medium">
                      Couleur : {v.color || "Non précisé"} • {v.year}
                    </p>
                    <div className="flex gap-2 mt-3 flex-wrap">
                      <span className="px-3 py-1 bg-zinc-100 border border-zinc-200 text-zinc-700 text-xs font-bold rounded-lg uppercase tracking-wider">
                        {v.licensePlate}
                      </span>
                      <span className="px-3 py-1 bg-primary/10 text-primary text-xs font-bold rounded-lg">
                        {v.seatsCapacity} Places passagers
                      </span>
                    </div>
                    {v.features.length > 0 && (
                      <div className="flex gap-2 mt-3 flex-wrap">
                        {v.features.map((f) => (
                          <span
                            key={f}
                            className="text-xs font-medium text-zinc-400 bg-zinc-50 px-2 py-1 rounded-md"
                          >
                            {f}
                          </span>
                        ))}
                ...
                <button
                  onClick={() => {
                    if (
                      confirm("Confirmez-vous la suppression de ce véhicule ?")
                    ) {
