  ...
  return (
    <main className="min-h-screen bg-light-300 flex items-center justify-center p-4 sm:p-8">
      <div className="w-full max-w-6xl bg-white rounded-[3rem] shadow-[0_30px_60px_rgba(0,0,0,0.06)] flex flex-col lg:flex-row-reverse overflow-hidden min-h-[750px]">
        ...
        <div ref={imageRef} className="hidden lg:flex flex-col relative w-1/2 bg-dark">
          <Image 
          ...
          <div className="relative z-10 p-14 h-full flex flex-col">
            <Link href="/" className="w-12 h-12 bg-white/20 rounded-full flex items-center justify-center backdrop-blur-md text-white hover:bg-white/40 transition-colors self-end">
            ...
            <div className="mt-auto">
              ...
              <p className="text-white/90 font-medium text-lg max-w-md">
                Rejoignez des milliers de conducteurs et passagers dans un environnement de confiance.
              </p>
            </div>
          </div>
        </div>
        ...
        {/* Left Column - Form */}
        <div ref={formRef} className="w-full lg:w-1/2 p-8 sm:p-14 flex flex-col justify-center relative">
          
          <Link href="/" className="lg:hidden absolute top-8 left-8 w-12 h-12 bg-light-300 rounded-full flex items-center justify-center text-dark hover:bg-light-400 transition-colors">
            <IoChevronBack size={24} />
          </Link>

          <div className="max-w-md w-full mx-auto">
            <h2 className="text-3xl lg:text-4xl font-black text-dark mb-2">Créer un compte</h2>
            <p className="text-neutral-500 font-medium mb-10">Rejoignez la communauté Wigo Express en quelques clics.</p>
            ...
            >
              {isGoogleLoading ? (
                 <div className="w-5 h-5 border-2 border-primary/30 border-t-primary rounded-full animate-spin"></div>
                  ...
                  <Image src="/images/google-icon-logo.svg" alt="Google" width={24} height={24} />
              )}
            ...
            <div className="flex items-center gap-4 my-8">
              ...
              <span className="text-xs font-bold text-neutral-400 uppercase tracking-widest">ou par e-mail</span>
              <div className="flex-1 h-px bg-neutral-100"></div>
            </div>

            <form onSubmit={handleRegister} className="flex flex-col gap-4">
              <div className="relative group">
                <IoPersonOutline className="absolute left-4 top-1/2 -translate-y-1/2 text-neutral-400 group-focus-within:text-primary transition-colors" size={22} />
                <input 
                   type="text" 
                   required
                   value={name}
              ...
              <div className="relative group">
                <IoMailOutline className="absolute left-4 top-1/2 -translate-y-1/2 text-neutral-400 group-focus-within:text-primary transition-colors" size={22} />
                <input 
                   type="email" 
                   required
                   value={email}
              ...
              <div className="relative group">
                <IoLockClosedOutline className="absolute left-4 top-1/2 -translate-y-1/2 text-neutral-400 group-focus-within:text-primary transition-colors" size={22} />
                <input 
                   type="password" 
                   required
                   value={password}
            ...
            <p className="text-center font-medium text-neutral-500 mt-10">
              Déjà un compte ? <Link href="/login" className="font-bold text-primary hover:text-dark transition-colors">Se connecter</Link>
            </p>
          </div>
        </div>
