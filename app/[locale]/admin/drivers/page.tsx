...
export default function AdminDriversPage() {
  ...
  const filteredRequests = driverRequests?.filter((req: any) => 
    filter === "ALL" ? true : (filter === "PENDING" ? !req.user.isDriver : req.user.isDriver)
  );
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-1">
          ...
          <p className="text-neutral-500 text-sm font-medium">
            Examinez les permis de conduire pour valider les conducteurs.
          </p>
        </div>
        ...
        <div className="flex items-center gap-1 bg-white p-1 rounded-xl border border-neutral-200 shadow-sm">
          {["ALL", "PENDING", "APPROVED"].map((f) => (
            ...
            >
              {f === "ALL" ? "Tout" : (f === "PENDING" ? "Attente" : "Validés")}
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 bg-white rounded-xl border border-neutral-200 shadow-sm">
            <MdDirectionsCar size={32} className="text-neutral-300 mb-4" />
                ...
                <Link href={`/admin/drivers/${request.id}`} className="relative h-40 w-full bg-neutral-50 overflow-hidden">
                  {request.documentUrl ? (
                    ...
                    <div className="w-full h-full flex flex-col items-center justify-center text-neutral-300 gap-2">
                      <MdDirectionsCar size={32} className="opacity-40" />
                      <span className="text-[9px] font-black uppercase tracking-widest opacity-60">Pas d'image</span>
                    </div>
                  )}
                  ...
                  <Link href={`/admin/drivers/${request.id}`} className="flex items-center gap-3 mb-4 group/user">
                    <div className="relative w-10 h-10 rounded-lg overflow-hidden ring-1 ring-neutral-100">
                      {request.user.image ? (
                        ...
                        <div className="w-full h-full bg-secondary-50 text-secondary-600 flex items-center justify-center text-sm font-bold">
                          {request.user.name[0]}
                        </div>
                      )}
                    </div>
                  ...
                  <div className="mt-auto space-y-4">
                    ...
                    <div className="flex items-center justify-between text-[10px] font-bold">
                      ...
                      <span className={`px-2 py-0.5 rounded uppercase tracking-widest text-[9px] ${
                        request.user.isDriver ? "bg-emerald-50 text-emerald-600 border border-emerald-100" : "bg-amber-50 text-amber-600 border border-amber-100"
                      }`}>
