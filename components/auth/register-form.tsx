...
export function RegisterForm() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    if (!result.success) {
      setError(
        "Veuillez remplir correctement tous les champs. (Mot de passe min. 6 char.)",
      );
      setIsPending(false);
      return;
    }
    ...
    try {
      const resp = await register(result.data, callbackUrl);
      if (resp?.error) {
        setError(resp.error);
      }
    } catch (err) {
  ...
  return (
    <div className="w-full max-w-md p-8 bg-white rounded-4xl shadow-[0_20px_50px_-12px_rgba(0,0,0,0.1)] border border-black/5 mx-auto relative overflow-hidden">
      <Link
        href="/create"
        className="absolute top-6 left-6 w-10 h-10 bg-neutral-100 rounded-xl flex items-center justify-center text-neutral-500 hover:bg-black hover:text-white transition-colors"
      >
        <ArrowLeft className="w-4 h-4" />
      </Link>

      <div className="mb-10 mt-8 text-center flex flex-col items-center">
        <h1 className="text-3xl font-bold text-black mb-2 tracking-tighter uppercase">
          Créer un compte.
        </h1>
        <p className="text-neutral-500 text-sm font-medium">
          Générez des vidéos sans limite
        </p>
      </div>
