            ...
            <div className="flex flex-col relative px-2">
              ...
              <div className="flex gap-4 md:gap-6 relative group">
                ...
                <div className="flex-1 pb-10">
                  <div className="flex flex-col sm:flex-row sm:items-baseline gap-2 mb-1">
                    <span className="text-lg font-black text-dark">{ride.departure.time.split(' ')[0]}</span>
                    <h3 className="text-lg font-bold text-dark">{ride.departure.city}</h3>
                  </div>
                  <p className="text-base font-medium text-neutral-600 mb-1">{ride.departure.address}</p>
                  <p className="text-sm text-neutral-400 font-medium leading-relaxed max-w-md">{ride.departure.description}</p>
                </div>
              </div>
              ...
              <div className="flex gap-4 md:gap-6 relative group pt-2">
                ...
                <div className="flex-1">
                  <div className="flex flex-col sm:flex-row sm:items-baseline gap-2 mb-1">
                    <span className="text-lg font-black text-dark">{ride.arrival.time.split(' ')[0]}</span>
                    <h3 className="text-lg font-bold text-dark">{ride.arrival.city}</h3>
                  </div>
                  <p className="text-base font-medium text-neutral-600 mb-1">{ride.arrival.address}</p>
                  <p className="text-sm text-neutral-400 font-medium leading-relaxed max-w-md">{ride.arrival.description}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="bg-white rounded-3xl p-6 shadow-[0_20px_40px_rgba(0,0,0,0.04)] border border-neutral-100 flex items-center gap-5 group cursor-pointer hover:border-primary/20 transition-colors">
              ...
              <div className="text-left flex-1">
                <h4 className="text-base font-bold text-dark">{ride.driver.name}</h4>
                <div className="flex items-center gap-1 text-sm font-bold text-neutral-500 mt-1">
                  ...
                  <span className="text-neutral-400 hidden sm:inline">({ride.driver.reviewsCount} avis)</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-3xl p-0 shadow-[0_20px_40px_rgba(0,0,0,0.04)] border border-neutral-100 flex flex-col sm:flex-row relative overflow-hidden group min-h-[140px]">
              <div className="p-6 sm:w-1/2 flex flex-col justify-center relative z-10">
                <h4 className="text-sm font-bold text-neutral-400 uppercase tracking-widest mb-1">Véhicule</h4>
                <p className="text-lg font-bold text-dark">{ride.car.make} {ride.car.model}</p>
                <p className="text-sm font-medium text-neutral-500">{ride.car.color} • Année {ride.car.year}</p>
              </div>
              {ride.car.photo ? (
                <div className="relative h-40 sm:h-auto sm:w-1/2 bg-neutral-100">
                  <Image src={ride.car.photo} alt={ride.car.make} fill className="object-cover transition-transform duration-700" />
                </div>
              ) : (
            ...
            <div className="bg-white rounded-3xl p-6 shadow-[0_20px_40px_rgba(0,0,0,0.04)] border border-neutral-100 flex items-center gap-4">
              ...
              <div>
                <h4 className="text-base font-bold text-dark">Arrêts supplémentaires</h4>
                <p className="text-sm font-medium text-neutral-500">
                  Cet itinéraire compte {ride.stops.length > 0 ? "plusieurs" : "zéro"} arrêts.
                </p>
              </div>
            </div>
            ...
            <div className="bg-white rounded-3xl p-6 shadow-[0_20px_40px_rgba(0,0,0,0.04)] border border-neutral-100 flex items-center gap-4">
              ...
              <div>
                <h4 className="text-base font-bold text-dark">Politique d'annulation</h4>
                <p className="text-sm font-medium text-neutral-500 leading-snug line-clamp-2" title={ride.policy.description}>
          ...
          <div className="bg-dark rounded-4xl squircle p-8 shadow-[0_20px_40px_rgba(0,0,0,0.12)] border border-neutral-800 text-white relative overflow-hidden">
            ...
            <div className="mb-8 relative z-10">
              <div className="flex items-end gap-2 mb-2">
                <span className="text-5xl font-black text-white leading-none">{Math.floor(ride.price)}</span>
                <span className="text-2xl font-bold">{ride.currency}</span>
                <span className="text-sm font-medium text-neutral-400 mb-1 ml-1 uppercase tracking-wider">par place</span>
              </div>
              ...
              <div className="flex gap-2 mt-4">
                {ride.amenities.instantBooking && (
                  <span className="flex items-center gap-1.5 text-[11px] font-bold text-dark bg-white px-3 py-1.5 rounded-full uppercase tracking-widest shadow-lg shadow-white/10">
                    <IoFlashOutline size={15} className="shrink-0 -translate-y-[0.5px]" /> Instantané
                  </span>
                )}
            ...
            <div className="mb-8 space-y-4 relative z-10">
              <h4 className="text-[11px] font-bold text-neutral-400 uppercase tracking-widest">Ce qui est inclus</h4>
              ...
              <div className="flex flex-wrap gap-2">
                {ride.amenities.luggage && (
                  <div className="px-3 py-2 bg-neutral-800/50 rounded-xl border border-neutral-700/50 flex items-center gap-2">
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-neutral-300"><path d="M6 20h12a2 2 0 0 0 2-2V8a2 2 0 0 0-2-2H6a2 2 0 0 0-2 2v10a2 2 0 0 0 2 2z"></path><path d="M8 18V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v14"></path><path d="M10 20h4"></path></svg>
                    <span className="text-sm font-medium text-neutral-300">Bagage max</span>
                  </div>
                )}
                ...
                {!ride.amenities.smoking && (
                  <div className="px-3 py-2 bg-neutral-800/50 rounded-xl border border-neutral-700/50 flex items-center gap-2">
                    <div className="relative">
                      <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-neutral-500"><path d="M12 12a2 2 0 1 0 0-4 2 2 0 0 0 0 4z"></path><path d="M12 18a2 2 0 1 0 0-4 2 2 0 0 0 0 4z"></path></svg>
                    ...
                    <span className="text-sm font-medium text-neutral-500">Non Fumeur</span>
                  </div>
                )}
                 {!ride.amenities.pet && (
                  <div className="px-3 py-2 bg-neutral-800/50 rounded-xl border border-neutral-700/50 flex items-center gap-2">
                    <div className="relative">
                      <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-neutral-500"><path d="M12 5c2 0 2-3 2-3s0 3 2 3c1.7 0 2 1.3 2 3 0 1.5-.7 3-1.4 4-.6.9-1.6 1.5-2.6 1.7V22H10v-5.2c-1 .2-2-.8-2.6-1.7-.7-1-1.4-2.5-1.4-4 0-1.7.3-3 2-3z"></path></svg>
                      <div className="absolute top-0 left-0 w-full h-full border-t border-neutral-500 rotate-45 transform origin-center translate-y-[8px]"></div>
                    </div>
                    <span className="text-sm font-medium text-neutral-500">Animaux interdits</span>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="bg-white/5 border border-white/10 rounded-2xl p-4 mb-8 flex gap-3 relative z-10 hover:bg-white/10 transition-colors">
              <IoCardOutline size={24} className="text-neutral-400 shrink-0 mt-0.5" />
              <p className="text-xs font-medium text-neutral-300 leading-relaxed">
                La contribution de {Math.floor(ride.price)}$ est préautorisée sur votre carte. Une partie du montant est reversée en cas d'annulation de dernière minute. (frais: {ride.bookingFee}$)
              </p>
            </div>
              ...
              <div className="relative h-14">
                ...
                >
                  <span className="text-sm font-bold text-white">
                    Réserver {seatsToReserve} place{seatsToReserve > 1 ? 's' : ''}
                  </span>
                  <IoChevronDown className={`text-white transition-transform ${isDropdownOpen ? "rotate-180" : ""}`} />
                </button>
                {isDropdownOpen && (
