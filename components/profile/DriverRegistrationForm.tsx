...
export function DriverRegistrationForm({ kycVerifications, license }: Props) {
  ...
  if (kycStatus === "PENDING") {
    return (
      <div className="bg-white border border-neutral-100 rounded-3xl p-8 shadow-[0_30px_60px_rgba(0,0,0,0.04)] flex flex-col justify-center h-full">
        <h3 className="text-2xl font-black mb-4 text-dark">Devenir Conducteur</h3>
        <div className="p-6 bg-amber-50 border border-amber-100 rounded-3xl text-amber-800">
          ...
          <p className="mt-2 text-[15px] text-amber-700/90 font-medium">Votre permis de conduire est en cours d'examen par notre équipe. Nous vous tiendrons informé sous 24h.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white border border-neutral-100 rounded-3xl p-8 lg:p-10 shadow-[0_30px_60px_rgba(0,0,0,0.04)] flex flex-col h-full">
      <h3 className="text-2xl font-black mb-2 text-dark">Devenir Conducteur</h3>
      <p className="text-neutral-500 text-[15px] mb-8 font-medium leading-relaxed">
        Ajoutez les informations de votre permis de conduire pour pouvoir proposer vos propres trajets en toute sécurité.
      </p>
      
      <form onSubmit={handleSubmit} className="flex flex-col gap-6 flex-1">
        <div>
          <label className="block text-[13px] font-bold text-neutral-500 uppercase tracking-wider mb-2">Numéro de permis</label>
          <input 
            type="text" 
            required
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-[13px] font-bold text-neutral-500 uppercase tracking-wider mb-2">Expiration</label>
            <input 
              type="date" 
              required
          ...
          <div>
            <label className="block text-[13px] font-bold text-neutral-500 uppercase tracking-wider mb-2">Pays</label>
            <select 
        ...
        <div>
          <label className="block text-[13px] font-bold text-neutral-500 uppercase tracking-wider mb-2">Photo de permis (URL test)</label>
          <input 
            type="text" 
            className="w-full bg-light-300 border-none rounded-2xl px-5 py-4 focus:outline-none focus:ring-2 focus:ring-primary text-dark font-bold transition-all text-base placeholder:text-neutral-400 placeholder:font-medium"
